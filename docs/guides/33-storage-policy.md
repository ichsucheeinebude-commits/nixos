...
path: "root/guides/33-storage-policy.md"
links:
  adr: ADR-33-storage-policy.md
  guide: 33-storage-policy.md
...
This module provides "storage policy" functionality for the NixOS system.
...
my.services."storage-policy".enable = true;
```
...
nixos-option my.services."storage-policy".enable

# Check logs
journalctl -u "storage-policy" -f --no-pager
```
...
- **Log location:** `journalctl -u "storage-policy" -f`
